...
## Outcome (recorded manually by brain session)

- Outcome: failure — moved to failed by hand
- Attempt 1: triage routed to codex, which returned the result-contract schema
  verbatim instead of a filled result (job failed in 15s)
- Attempt 2 (retry 1): job created 20:07:52Z, watcher never recorded an
  outcome, job vanished — issue #18 territory (same job name + dedup)
- Re-dispatched as cloud-burst-pool-matching-v2 with agent pinned to claude
